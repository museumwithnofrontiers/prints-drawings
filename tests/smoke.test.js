...
describe('website smoke test', () => {
  ...
  it('renders the about page on the composed text page view', async () => {
    ...
    expect(host.textContent).toContain("Prints and drawings")
  ...
  it('colours and names the source-database chip from the manifest projects section', async () => {
    const { app, host } = await mountSite('#/item/919aef03-f815-592e-ab38-dd26eb510ac9')
  ...
  it('shows the explore-partner notice only for the project dataset.config.js lists', async () => {
    const epm = await mountSite('#/item/e6775e8c-d02b-5493-bf5e-5bcf66f6cbb0')
    ...
    const isl = await mountSite('#/item/919aef03-f815-592e-ab38-dd26eb510ac9')
  ...
  it('renders the related-database and artistic-introduction links from the manifest', async () => {
    const [items] = await loadEntities(['items'])
    const item = items.find((i) => i.id === '919aef03-f815-592e-ab38-dd26eb510ac9')
  ...
  it('shows the source project on a collection-results tile, from the manifest', async () => {
    ...
    const item = items.find((i) => i.project_id === islProjectId) ?? { internal_name: "Tughra (imperial monogram)." }
  ...
  it('renders the layout glossary tool and dynasty popouts on the item sheet', async () => {
    ...
    const { app, host } = await mountSite('#/item/23e8db58-0148-5dc3-98b4-98cb5313ba16')
    ...
    expect(host.textContent).toContain("Ayyubids")
    app.unmount()
  }, 60000)
  ...
  it('renders the partners list on the composed partner-list view', async () => {
    ...
    expect(host.textContent).toContain('Municipal Library at the Archiginnasio Palace')
  ...
  it('renders a partner profile on the composed record view', async () => {
    const { app, host } = await mountSite('#/partner/7cbc59ff-26f9-5d77-9853-2e0324c66c08')
    ...
    await vi.waitFor(() => expect(host.textContent).toContain('Municipal Library at the Archiginnasio Palace'), { timeout: 20000 })
    await vi.waitFor(() => expect(host.textContent).toContain("Bologna"), { timeout: 20000 })
  ...
  it("renders a partner's objects on the composed grid results view", async () => {
    const { app, host } = await mountSite('#/partner/7cbc59ff-26f9-5d77-9853-2e0324c66c08/objects')
  ...
  it('renders the shared texts and its own over them', async () => {
    ...
    expect(checkTextsRendered(host, {
      namespaces: ['printsDrawings', 'core', 'layout', 'catalogue', 'record', 'sheet', 'timeline', 'partner', 'gallery'],
    })).toEqual([])
