...
<template>
  <SiteShell
    v-bind="$attrs"
    header-home="#/"
    :footer-text="t('printsDrawings.identity.copyright')"
  >
    <template #brand>
      <span class="site-logo">
        <span class="site-logo-org">{{ t('printsDrawings.identity.organisation') }}</span>
        <span class="site-logo-title">{{ t('printsDrawings.identity.title') }}</span>
      </span>
    </template>
    <slot />
  </SiteShell>
